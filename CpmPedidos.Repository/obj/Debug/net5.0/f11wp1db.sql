...
CREATE TABLE tb_promocaoproduto (
    id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    nome character varying(50) NOT NULL,
    preco numeric(17,2) NOT NULL,
    id_imagem integer NOT NULL,
    ...
    ativado boolean NOT NULL,
    criado_em timestamp without time zone NOT NULL,
    CONSTRAINT "PK_tb_promocaoproduto" PRIMARY KEY (id),
    CONSTRAINT "FK_tb_promocaoproduto_tb_imagem_id_imagem" FOREIGN KEY (id_imagem) REFERENCES tb_imagem (id) ON DELETE CASCADE,
    CONSTRAINT "FK_tb_promocaoproduto_tb_produto_id_produto" FOREIGN KEY (id_produto) REFERENCES tb_produto (id) ON DELETE CASCADE
);
...
CREATE TABLE tb_produtopedido (
    ...
    criado_em timestamp without time zone NOT NULL,
    CONSTRAINT "PK_tb_produtopedido" PRIMARY KEY (id),
    CONSTRAINT "FK_tb_produtopedido_tb_pedido_id_pedido" FOREIGN KEY (id_pedido) REFERENCES tb_pedido (id) ON DELETE CASCADE,
    CONSTRAINT "FK_tb_produtopedido_tb_produto_id_produto" FOREIGN KEY (id_produto) REFERENCES tb_produto (id) ON DELETE CASCADE
);
...
CREATE INDEX "IX_tb_produtopedido_id_pedido" ON tb_produtopedido (id_pedido);

CREATE INDEX "IX_tb_produtopedido_id_produto" ON tb_produtopedido (id_produto);

CREATE INDEX "IX_tb_promocaoproduto_id_imagem" ON tb_promocaoproduto (id_imagem);

CREATE INDEX "IX_tb_promocaoproduto_id_produto" ON tb_promocaoproduto (id_produto);

INSERT INTO "__EFMigrationsHistory" ("MigrationId", "ProductVersion")
VALUES ('20210516024852_InitialCreate', '5.0.6');
